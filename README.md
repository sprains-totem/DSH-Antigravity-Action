...
- [x] 组合测试验证（4 种发起/结束方式）
- [x] M1: Android 连接 + 握手（ClassicBtTransport + QwenHandshake 状态机）
- [x] M2: 录音开始/结束 + 音频落盘（398B 帧解析 → WAV，5min 切片）
- [x] M3: 录音列表 + 播放器（播放/删除/分享 + 变速/±10s/循环）
- [ ] M4: 稳定性（真机重连 / 断流 / 长录音 / 后台实测）
- [ ] M5: 扩展（AAC、分享 / ZIP、云同步）
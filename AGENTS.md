...
1. **Slot A 存档**：启动前，脚本自动将当前的 `cordis.patch.yml`、`plugins/` 和 `unlock-dsh.mjs` 备份至 `~/.dsh/slots/slot-a`；
2. **15 秒健康判定**：启动 `dsh web` 后，守护循环进行 15 秒心跳检测；
3. **自动回滚**：如果进程在 15 秒内异常退出（Exit Code $\ne 0$）：
   - 捕获最后 20 行崩溃堆栈；
   - 立即将 Slot A 稳定快照覆盖回工作区与 `~/.dsh/profiles/web/`；
   - 重新以稳定模式拉起服务，输出警报并自愈，保障 Web 永远在线；
4. **晋升机制**：稳定运行超过 15 秒后，当前代码自动晋升为新的 Slot A 稳定版本。

...
1. **🚫 严禁越权干预自愈槽位 (`~/.dsh/slots/`)**：
   ...
   - **绝对禁止** Agent 使用任何手段手动覆盖、修改或删除 `~/.dsh/slots/` 中的内容；
   - 晋升与回滚只能由 `start.sh` 在通过端到端深度健康检测后全自动执行。
2. **🚫 严禁臆断「服务没崩即配置正常」**：
   - `dsh-fail-soft` 会全局捕获未处理的异常，插件加载失败或 Schema 错误**不会导致 Node 进程退出**；
   - 任何涉及功能变更（如搜索源、模型参数、插件注册）的修改，**必须进行端到端语义级验证**（调用工具测试、HTTP 握手、CDP 探针），严禁凭「进程还活着」单方面宣称完成。
3. **🚫 严禁在 GitHub Actions 运行期修改工作流文件**：
   - 严禁直接改动 `.github/workflows/`；所有逻辑、依赖与生命周期调整统一在 `start.sh` 中完成。
4. **🔒 工作区隔离与稳定版本固化原则**：
   - 未提交的代码一律视为待验证的候选态（Candidate / Dirty）；
   - 只有经过 Git Commit 确认的干净版本才具备晋升为稳定基准的资格。

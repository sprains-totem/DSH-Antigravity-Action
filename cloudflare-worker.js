/**
 * DeepSeek Harness - 带密码保护的动态路由 Cloudflare Worker
 * 
 ...
 * 3. AUTH_TOKEN       : Action 上报专用 Token（如：cf_token_dsh_2026）
 */
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/login") {
      ...
      if (pwd === visitPassword) {
        ...
        if (env.DSH_KV) {
          ...
          if (raw) {
            try { targetUrl = JSON.parse(raw).url; } catch(e) {}
          }
        }
    ...
    if (url.pathname === "/json" || request.headers.get("accept")?.includes("application/json")) {
      if (!data) return new Response(JSON.stringify({ status: "offline" }), { status: 404 });
      return new Response(JSON.stringify(data, null, 2), { headers: { "Content-Type": "application/json" } });
    }

    // 纯文本 URL: GET /url
    if (url.pathname === "/url") {
      if (!data) return new Response("offline", { status: 404 });
      return new Response(data.url, { headers: { "Content-Type": "text/plain" } });
    }

    // 默认行为：若带 ?status=1 参数则展示状态页，否则直接 302 重定向到 Harness
    if (data && data.url) {
      if (url.searchParams.get("status") === "1") {
        return new Response(renderStatusHtml(data), { headers: { "Content-Type": "text/html;charset=UTF-8" } });
      }
...
<body>
  <div class="card">
    ...
    <h1>DeepSeek Harness</h1>
    <p>最新临时隧道地址：</p>
    <p style="background: #0f172a; padding: 0.75rem; border-radius: 0.5rem; font-family: monospace; color: #38bdf8;">${data.url}</p>
    <a href="${data.url}" class="btn">进入 Web 页面</a>

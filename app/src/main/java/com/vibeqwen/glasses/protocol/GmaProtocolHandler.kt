...
object GmaProtocolHandler {
    ...
    fun handleIncomingBytes(bytes: ByteArray): ByteArray? {
        ...
        // 2. GMA 二进制消息解析 (如 01 00 09 20 00 1C 03 0B 00 00 00 00)
        // 典型结构：[0..1] CID, [2..3] Len/Flag, [4..5] CmdId, [6] NameSpace, [7] MsgId, [8..] Payload
        try {
            val cid = (bytes[0].toInt() and 0xFF) or ((bytes[1].toInt() and 0xFF) shl 8)
            if (cid == 0x0001 || cid == 0x0041 || cid == 0x004A) {
                val cmdId = (bytes[4].toInt() and 0xFF) or ((bytes[5].toInt() and 0xFF) shl 8)
                val ns = bytes[6].toInt() and 0xFF
                val msgId = bytes[7].toInt() and 0xFF
                LogCollector.p("GMA 二进制命令: CID=0x%04X, ns=0x%02X, cmd=0x%04X, msgId=0x%02X".format(cid, ns, cmdId, msgId))

                // 自动构造 GMA 成功应答包 (ACK)
                return buildGmaAck(cid = cid, ns = ns, cmdId = cmdId, msgId = msgId)
            }
        } catch (e: Exception) {
